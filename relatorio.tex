...
% Pacotes usados pelo relatório e pelas tabelas geradas pelo R.
...
% Evita cola entre blocos de rodapé quando uma longtable é dividida entre páginas.
\makeatletter
\patchcmd{\LT@output}
{\copy\LT@foot\vss}
...
% Macros dinâmicas geradas pelo R.
\input{outputs/macros.tex}
\input{outputs/macros_analises.tex}
...
\begin{flushright}
  ...
  {\normalsize Consultor: \rule[-0.05cm]{3.2cm}{0.4pt}}\\
  {\normalsize Requisitante: \rule[-0.05cm]{3.2cm}{0.4pt}}\\
...
associada à obesidade, diabetes mellitus tipo 2 e síndrome metabólica
\cite{buzzetti2016,eslam2020,easl2024}. Estudos
...
intervalo de confiança (IC) de 95\% pelo intervalo score de Wilson com correção
de continuidade. Para o objetivo 2, a
...
Todas as análises foram realizadas em R 4.6.1 \cite{r2026}; as figuras que usam
jitter foram geradas com semente fixa para garantir a reprodutibilidade.
...
\begin{table}[H]
  ...
  \input{outputs/tabela_descritiva_numericas.tex}
...
\input{outputs/tabela_descritiva_categoricas.tex}
...
\input{outputs/tabela_cruzamento_DHEM.tex}
...
frequência de baixa massa magra em cada critério, com IC de 95\% (intervalo
score de Wilson com correção de continuidade). A prevalência variou de 7,3\%
(Baumgartner) a 44,8\% (Newman):
...
\begin{table}[H]
  ...
  \caption{Prevalência de baixa massa magra segundo os critérios de MMEA, com
           intervalo de confiança de 95\% (intervalo score de Wilson com
           correção de continuidade).}
  \label{tab:prevalencia_ic}
  \input{outputs/tabela_prevalencia_IC.tex}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_concordancia_ajustes.tex}
...
\input{outputs/tabela_triagem_fibrose.tex}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_modelo_simples_fibrose.tex}
\end{table}
...
(sexo, idade e IMC) em um modelo de regressão linear múltiplo, a fibrose
...
\begin{table}[H]
  ...
  \input{outputs/tabela_modelo_fibrose.tex}
...
\input{outputs/tabela_triagem_dhem.tex}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_modelo_simples_dhem.tex}
\end{table}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_modelo_dhem.tex}
\end{table}
...
\input{outputs/tabela_comparacao_grupos.tex}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_posthoc_grupos.tex}
\end{table}
...
\begin{thebibliography}{9}
  \bibitem{buzzetti2016} Buzzetti E, Pinzani M, Tsochatzis EA. The multiple-hit pathogenesis of
        non-alcoholic fatty liver disease (NAFLD). \textit{Metabolism}.
        2016;65(8):1038--1048.
  \bibitem{eslam2020} Eslam M, Newsome PN, Sarin SK, et al. A new definition for
  ...
  \bibitem{riazi2022} Riazi K, Azhari H, Charette JH, et al. The prevalence and incidence of
        NAFLD worldwide: a systematic review and meta-analysis.
        \textit{J Hepatol}. 2022;77(5):1335--1347.
  \bibitem{easl2024} EASL--EASD--EASO. Clinical Practice Guidelines on the management of
        metabolic dysfunction-associated steatotic liver disease (MASLD).
        \textit{J Hepatol}. 2024.
  \bibitem{ada2025} American Diabetes Association. Standards of Care in Diabetes--2025.
        \textit{Diabetes Care}. 2025;48(Suppl 1).
  \bibitem{sbgg2022} Sociedade Brasileira de Geriatria e Gerontologia (SBGG). \textit{Manual
        de Recomendações para Diagnóstico e Tratamento da Sarcopenia no
        Brasil}. 2022.
  \bibitem{sarcopenia_masld2025} Sarcopenia and MASLD: novel insights and the future.
        \textit{Nat Rev Endocrinol}. 2025. doi:10.1038/s41574-025-01197-7.
  \bibitem{r2026} R Core Team. \textit{R: A Language and Environment for Statistical
...
\clearpage
\appendix
\section{Diagnóstico dos modelos de regressão logística}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_diagnosticos_modelos.tex}
\end{table}
...
\begin{table}[H]
  ...
  \input{outputs/tabela_diagnosticos_concordancia.tex}
\end{table}
